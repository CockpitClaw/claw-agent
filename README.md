...
- **Vehicle native apps may be covered by an overlay**: some vehicles (e.g. Li Auto) run a persistent translucent sidebar Activity (`com.XXX.sidebar`) that sits above newly-launched apps, hiding them from the foreground even though the app is actually running and its UI is still readable via accessibility. This is vehicle-system behavior, not a project issue.
